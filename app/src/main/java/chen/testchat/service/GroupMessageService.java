...
public class GroupMessageService extends Service {
        ...
        private void dealListener() {
                ...
                data.start(CustomApplication.getInstance(), new ValueEventListener() {
                                @Override
...
//                                                                不接受本用户发过来的消息
                                                        ...
                                                        if (!CommonUtils.isAppOnForeground(getBaseContext())) {
                                                                if (CustomApplication.getInstance().getSharedPreferencesUtil().isRemind(groupChatMessage.getGroupId())) {
