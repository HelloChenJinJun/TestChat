...
public class HttpServiceManager {
        ...
        private static File cacheFileDir = new File(CustomApplication.getInstance().getCacheDir(), "tx_cache");
        ...
        public TxApi getTxApi() {
                if (sTxApi == null) {
                        synchronized (monitor) {
                                if (sTxApi == null) {
                                        sTxApi = new Retrofit.Builder().client(client).baseUrl(TxBaseUrl).addCallAdapterFactory(RxJavaCallAdapterFactory.create())
        ...
        public HappyApi getHappyApi() {
                if (sHappyApi == null) {
                        synchronized (happy) {
                                if (sHappyApi == null) {
                                        sHappyApi = new Retrofit.Builder().client(client).baseUrl(happyBaseUrl).addCallAdapterFactory(RxJavaCallAdapterFactory.create())
        ...
        public PictureApi getPictureApi() {
                if (sPictureApi == null) {
                        synchronized (monitor) {
                                if (sPictureApi == null) {
                                        sPictureApi = new Retrofit.Builder().client(client).baseUrl(pictureBaseUrl).addCallAdapterFactory(RxJavaCallAdapterFactory.create())
