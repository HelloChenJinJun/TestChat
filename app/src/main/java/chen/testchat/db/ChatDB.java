...
//        *******************************   消息表的各项列名      //
        ...
        private ChatDB(String userId) {
                ...
                config.setContext(CustomApplication.getInstance());
        ...
        public long saveChatMessage(ChatMessage message) {
                ...
                if (mDatabase.isOpen()) {
                        ...
                        if (result > 0) {
                                LogUtil.e("保存或更新到数据库的聊天消息表成功");
                        } else {
                                LogUtil.e("保存或更新到数据库的聊天消息表失败");
                        }
                }
                return result;
        }
...
//                                如果该消息未存在的情况下
//                                只要更新最后一条的消息就行了
                                LogUtil.e("该用户的最近消息已经存在，损益只需更新最新的消息就可以拉");
...
//                                否则是从定时服务那里拉数据过来的，这里只需要更新时间就行了
                                LogUtil.e("从定时服务那里拉数据过来的，这里只需要更新时间就行了");
                        ...
                        if (result > 0) {
                                LogUtil.e("保存或更新消息到数据库中的最近会话列表成功");
                        } else {
                                LogUtil.e("保存或更新消息到数据库中的最近会话列表失败");
                        }
                }
                return result;
        }
        ...
        public long saveInvitationMsg(ChatMessage chatMessage) {
                ...
                if (mDatabase.isOpen()) {
                        if (!isExistInvitationMsg(chatMessage.getBelongId(), chatMessage.getCreateTime())) {
                                ...
                                if (result > 0) {
                                        LogUtil.e("保存邀请消息到数据库成功");
                                } else {
                                        LogUtil.e("保存邀请消息到数据库失败");
                                }
                        } else {
                                result = 1;
                        }
